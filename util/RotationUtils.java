...
public class RotationUtils
{
	...
	{
		float f = WMath.cos(-Minecraft.getMinecraft().player.rotationYaw * 0.017453292F
			- (float)Math.PI);
		float f1 = WMath.sin(-Minecraft.getMinecraft().player.rotationYaw * 0.017453292F
			- (float)Math.PI);
		float f2 =
			-WMath.cos(-Minecraft.getMinecraft().player.rotationPitch * 0.017453292F);
		float f3 =
			WMath.sin(-Minecraft.getMinecraft().player.rotationPitch * 0.017453292F);
	...
	private static float[] getNeededRotations2(Vec3d vec)
	{
		...
		return new float[]{
			Minecraft.getMinecraft().player.rotationYaw
				+ WMath.wrapDegrees(yaw - Minecraft.getMinecraft().player.rotationYaw),
			Minecraft.getMinecraft().player.rotationPitch + WMath
	...
	public static float getAngleToClientRotation(Vec3d vec)
	{
		...
		float diffYaw =
			WMath.wrapDegrees(Minecraft.getMinecraft().player.rotationYaw) - needed[0];
		float diffPitch =
			WMath.wrapDegrees(Minecraft.getMinecraft().player.rotationPitch) - needed[1];
